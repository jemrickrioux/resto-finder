    ...
    return (
      <Link href={link} passHref>
        <button type={"button"} onClick={action} className={styles}>
          <span>{text}</span>
          <ArrowRightIcon className=" bold transition ease-in-out group-hover:translate-x-1 ml-2 h-6 w-6 hover:transform" />
        </button>
      </Link>
    );
  }
  return <button onClick={action} type={submit ? "submit" : "button" } className={styles}>{text}</button>;
};
